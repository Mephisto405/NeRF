...
class NeRFDataset(Dataset):
    ...
    def __init__(self, images, poses, render_poses, hwf, i_split, mode='train', device='cuda:0'):
        ...
        #te_images = np.stack([images[i] for i in i_te])
        tr_poses = np.stack([poses[i] for i in i_tr])
        val_poses = np.stack([poses[i] for i in i_tr])
        #te_poses = np.stack([poses[i] for i in i_tr])
        ...
        if mode == 'train':
            ...
            self.ipt = self.init_ray_from_dataset(val_poses)
            self.ipt = torch.Tensor(self.ipt).to(device)
            self.gt = torch.Tensor(val_images.reshape(-1,3)).to(device)
            assert len(self.ipt) == len(self.gt), 'input dataset size: %d, gt dataset size: %d'%(self.ipt, self.gt)
        elif mode == 'test':
            self.ipt = self.init_ray_from_dataset(render_poses)
            self.ipt = torch.Tensor(self.ipt).to(device)
            #self.gt = torch.zeros((len(self.ipt), 3), device=device)
            #assert len(self.ipt) == len(self.gt), 'input dataset size: %d, gt dataset size: %d'%(self.ipt, self.gt)
        else:
    ...
    def init_ray_from_dataset(self, poses):
        ...
        rays = rays.reshape(-1, 2, 3) # (N*H*W, 2=o+d, 3)
        #ray_origins, ray_dirs = rays[:,0,:], rays[:,1,:]
        return rays
    ...
    def __len__(self):
        if self.mode == 'test':
            ...
            return self.ipt.shape[0] #64*64*8#
